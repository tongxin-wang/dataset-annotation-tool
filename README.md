...
> 注：我们要做的任务是推理出图像与文本之间的不匹配之处，原始数据的文本caption都是对图像的正确描述，我们的标注工作主要就是使用这个标注工具修改文本caption（替换句子中的形容词，动词，名词等...），同时标出图像相关区域，使得图像与文本之间存在`细粒度的语义差错`。  
简单来说就是：给的句子是对的，要改成错误的然后把错的地方标出来。
